...
const MAPS_GUIDE_HTML = `
    <div style="padding:24px;max-width:760px;max-height:calc(85vh - 48px);box-sizing:border-box;overflow-y:auto;text-align:left;font-family:var(--rt-font,system-ui,sans-serif);color:var(--SmartThemeBodyColor,inherit);line-height:1.55;">
        <h2 style="margin:0 0 18px;color:#7dd3fc;">Maps Guide</h2>
        <h3 style="margin:18px 0 6px;color:#bae6fd;">What Are Maps?</h3>
        <p>Maps are persistent “physical” locations in JSON that are generated by a dedicated Map Architect agent and updated by a Map Updater every turn you’re inside a map. They’re then optionally (though I recommend it as it makes them actually alive!) evolved by a Map Evolution agent periodically (adjustable interval).</p>
        <h3 style="margin:18px 0 6px;color:#bae6fd;">Why Do They Matter?</h3>
        <p>They matter especially because of the way they interact with skill checks. For example, without an underlying mapped reality, the GM may give you a choice such as, “Look for traps DC 12” or “See if you can spot any enemies DC 8.” If you choose such an option and you roll a success, you will find enemies; if you roll a failure, there will be no enemies.</p>
        <p>You see the problem? <strong>The trap or enemy is <em>spawned/created</em> by the roll.</strong> On the other hand, if there’s an underlying reality (the map), the check is actually compared to a physical reality. It’s a kind of dual-context system.</p>
        <p>It also makes everything spatially coherent. For example, if you collapse a ceiling on top of a pack of enemies, the Map Updater will mark the passage as closed and the enemy pack as destroyed. The way is actually closed, and you need to find a way around. The GM doesn’t just hallucinate infinite routes.</p>
