...
describe('Character creation chat ownership', () => {
    it('Quick Start refuses cancelled/chat-changed direct prompts and pins Player Card writes', () => {
        ...
        expect(characterCreatorSource).toContain("import { canCommitPassForChat } from './src/state/pass-affinity.js';");
        expect(gen).toContain('const passChatId = getActiveChatId();');
        expect(gen).toContain('assertDirectPromptOwned(result)');
        expect(gen).toContain('canCommitPassForChat(passChatId, getActiveChatId())');

        expect(quickStartSource).toContain("import { canCommitPassForChat } from './src/state/pass-affinity.js';");
        expect(quickStartSource).toContain('addPlayerCardToLorebookAgent(charName, bio, wordCount, { chatId: passChatId })');
    ...
    it('Character Creator and PC Import stop follow-up writes after affinity loss', () => {
        ...
        expect(roll).toContain('canCommitPassForChat(passChatId, getActiveChatId())');
        expect(roll).toMatch(/await generatePersonaBio\([\s\S]*?canCommitPassForChat\(passChatId, getActiveChatId\(\)\)/);
        ...
        expect(importFn).toMatch(/customPortraits[\s\S]*?canCommitPassForChat\(passChatId, getActiveChatId\(\)\)/);
    });
    ...
    it('addPlayerCardToLorebookAgent refuses writes when the originating chat is no longer live', () => {
        ...
        expect(add).toContain('canCommitPassForChat(passChatId, getActiveChatId())');
    ...
    it('onboarding archetype buttons require sendDirectPrompt success before persona follow-ups', () => {
        ...
        expect(personaFn).toContain('const passChatId = getActiveChatId();');
        expect(personaFn).toContain('canCommitPassForChat(passChatId, runtimeState.currentChatId)');
        expect(personaFn).toMatch(/await generatePersonaBio\([\s\S]*?canCommitPassForChat\(passChatId, runtimeState\.currentChatId\)/);
    });
});
